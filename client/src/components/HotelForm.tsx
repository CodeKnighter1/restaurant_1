...
const HotelForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await axios.post('http://localhost:5000/api/hotel/create', payload);
            navigate('/all-hotel');
        } catch (error: any) {
            const errorMessage =
                error.response?.data?.error || t('create_hotel_error');
    ...
    return (
        <div className='py-16 px-8 max-w-2xl mx-auto'>
            <h1 className='text-4xl font-bold text-center mb-4'>{t('add_hotel')}</h1>
            <p className='text-center text-gray-600 mb-10'>{t('add_hotel_description')}</p>
